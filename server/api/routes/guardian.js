...
function handleJson(json) {
    ...
    for (let i = 0; i < results.length;i++) {
        ...
        returns[count] = {"webTitle":results[i].webTitle,  "date":results[i].webPublicationDate,
            "description":results[i].blocks.body[0].bodyTextSummary, imgUrl, "id":results[i].id,url:results[i].webUrl};
...
function handleSearch(json) {
    ...
    for (let i = 0; i < results.length;i++) {
        ...
        returns[i] = {"title":results[i].webTitle,  "date":results[i].webPublicationDate.substring(0,10),
            "description":results[i].blocks.body[0].bodyTextSummary, "image":imgUrl, "id":results[i].id, "url":results[i].webUrl,"section":results[i].sectionId, "source":"guardian"};
